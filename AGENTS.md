...
| 2026-02-05 | Self-review before release (lasso input flow) | Combo was reset on invalid/empty lasso or non-lasso pointerup events | `pointerup` resolution ran even when no lasso was actually being drawn and treated invalid paths as a failure state | Added `LassoSelection.isDrawing()` guard, changed invalid/empty result handling to non-punitive feedback, and kept combo reset only on real mixed-type errors | Gate lasso resolution by explicit drawing state; do not apply gameplay penalties for UI/no-op pointer events |
| 2026-02-05 | Post-game restart flow (user bug report) | After game over, start overlay appeared but game could not start because click kept restarting scene | `SessionState` was not reset in `create()` after `scene.restart()`, so pointerdown stayed in `GameOver` branch | Explicitly reset `state = StartOverlay` in `GameScene.create()` before input flow starts | On every scene `create()` that supports restart, reset all transient gameplay state enums explicitly (do not rely on field initializers) |
| 2026-02-08 | Gameplay preview capture (`game-preview-webp` skill) | First WebP capture attempt failed or produced unusable output | Playwright browser launch was blocked in sandbox, and default host-platform detection targeted `mac-x64` paths in this environment | Re-ran recording with escalated permissions, set `PLAYWRIGHT_HOST_PLATFORM_OVERRIDE=mac15-arm64`, and used interactive capture mode to generate a valid clip | For Playwright-based capture in this environment, launch outside sandbox and set host platform override before recording |
